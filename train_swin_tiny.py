...
try:
    ...
    raise RuntimeError("未安装 timm，无法创建 Swin Transformer backbone。请先安装: pip install timm") from e
...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_swin_tiny',
    'save_dir': './checkpoints_swin_tiny',
    'num_classes': 4,
    'batch_size': 64,
    'lr': 1e-3,
    'epochs': 60,
...
# ================= 4. Swin-Tiny-based 模型实现（只换底座） =================
def _resolve_swin_model_name(model_name: str) -> str:
    try:
...
class BrainTumorSwinTiny(nn.Module):
    def __init__(self, num_classes=4, use_ca=True, use_symmetry=True, model_name='swin_tiny_patch4_window7_224'):
        super().__init__()
        self.use_symmetry = use_symmetry

        resolved_name = _resolve_swin_model_name(model_name)
        print(f">>> 初始化 Swin-Tiny | model={resolved_name} | CA: {use_ca} | Sym: {use_symmetry}")
    ...
    def _to_nchw(feat: torch.Tensor, expected_c: int) -> torch.Tensor:
        # timm 的部分 Swin features_only 输出为 NHWC，这里统一转成 NCHW
        if feat.ndim != 4:
            return feat
...
def train_one_experiment(config):
    ...
    model = BrainTumorSwinTiny(
        num_classes=config['num_classes'],
        use_ca=config['use_ca'],
        use_symmetry=config['use_symmetry'],
        model_name=config.get('swin_model_name', 'swin_tiny_patch4_window7_224'),
    ).to(device)
