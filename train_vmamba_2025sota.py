...
try:
    ...
    raise RuntimeError(
        "导入 VMamba 失败。\n"
        "常见原因：VMamba 依赖缺失，或其自定义算子在当前环境不可用。\n"
        f"原始错误: {e}"
    )
...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_vmamba_2025sota',
    'save_dir': './checkpoints_vmamba_2025sota',
    'num_classes': 4,

    # VMamba 通常更吃显存；默认略保守，必要时你可以再调大
    'batch_size': 4,
    'lr': 1e-3,
    'epochs': 60,
    'num_workers': 4,
    ...
    # VMamba 变体：tiny | small | base
    # 如果你确实要“冲 SOTA”，建议先试 base，但显存压力会更大
    'vmamba_variant': 'base',

    # 如果你有官方/自训预训练 ckpt 路径，可在这里填
    'vmamba_pretrained_ckpt': None,
}
...
class BrainTumorVMamba2025(nn.Module):
    def __init__(
        self,
        num_classes=4,
        use_dcn=True,
        ...
        if use_dcn:
            print("⚠️ VMamba 为序列/状态空间骨干，DCN 选项将被忽略。")
        ...
        # Backbone 输出 4 个 stage；取最后 3 个尺度 (stage 1/2/3)
...
def train_one_experiment(config):
    ...
    name_parts.append(f"VMamba_{config.get('vmamba_variant', 'base')}")
    ...
    for epoch in range(config['epochs']):
        ...
        if macro_f1 > best_macro_f1:
            ...
            with open(os.path.join(current_save_dir, txt_filename), "w", encoding="utf-8") as f:
                ...
                f.write(f"Backbone:         VMamba_{config.get('vmamba_variant', 'base')}\n")
