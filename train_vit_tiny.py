...
try:
    ...
    raise RuntimeError("未安装 timm，无法创建 ViT backbone。请先安装: pip install timm") from e
...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_vit_tiny',
    'save_dir': './checkpoints_vit_tiny',
    'num_classes': 4,
    'batch_size': 64,
    'lr': 1e-3,
    'epochs': 60,
    'num_workers': 4,  # 降低到 4，Windows 上减少开销
...
# ================= 4. ViT-Tiny-based 模型实现（只换底座） =================
def _resolve_vit_model_name(model_name: str) -> str:
    try:
...
class BrainTumorViTTiny(nn.Module):
    def __init__(self, num_classes=4, use_ca=True, use_symmetry=True, model_name='vit_tiny_patch16_224'):
        super().__init__()
        self.use_symmetry = use_symmetry

        resolved_name = _resolve_vit_model_name(model_name)
        print(f">>> 初始化 ViT-Tiny | model={resolved_name} | CA: {use_ca} | Sym: {use_symmetry}")

        # features_only 输出多个 block 特征，取 3 个尺度
        self.backbone = timm.create_model(
            resolved_name,
            pretrained=False,
            features_only=True,
            out_indices=(6, 9, 11),  # ViT-Tiny 有 12 个 blocks，取中间、后期、最后
        )

        try:
            self._feat_channels = list(self.backbone.feature_info.channels())  # [ch6, ch9, ch11]
        except Exception:
        ...
        ch6, ch9, ch11 = self._feat_channels

        self.ca6 = CoordAtt(ch6) if use_ca else nn.Identity()
        self.ca9 = CoordAtt(ch9) if use_ca else nn.Identity()
        self.ca11 = CoordAtt(ch11) if use_ca else nn.Identity()

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

        feat_dim = ch6 + ch9 + ch11
    ...
    def _to_nchw(feat: torch.Tensor, expected_c: int) -> torch.Tensor:
        # ViT features_only 输出可能是 (B, N, C) 或 (B, C, H, W)，需要统一转成 NCHW
        if feat.ndim == 3:
            # (B, N, C) -> reshape to (B, C, H, W)
            B, N, C = feat.shape
            H = W = int(N ** 0.5)
            if H * W == N:
                ...
                # 如果不是完美平方，补零或裁剪
                H = W = int(N ** 0.5)
                feat = feat[:, :H*W, :].transpose(1, 2).reshape(B, C, H, W)
    ...
    def forward_single_branch(self, x):
        feat6, feat9, feat11 = self.backbone(x)
        ch6, ch9, ch11 = self._feat_channels
        feat6 = self._to_nchw(feat6, ch6)
        feat9 = self._to_nchw(feat9, ch9)
        feat11 = self._to_nchw(feat11, ch11)

        feat6 = self.ca6(feat6)
        feat9 = self.ca9(feat9)
        feat11 = self.ca11(feat11)
        return feat6, feat9, feat11

    def forward(self, x):
        f6, f9, f11 = self.forward_single_branch(x)
        f6 = self.avgpool(f6).flatten(1)
        f9 = self.avgpool(f9).flatten(1)
        f11 = self.avgpool(f11).flatten(1)
        feat_origin = torch.cat([f6, f9, f11], dim=1)

        if not self.use_symmetry:
            return self.classifier(feat_origin)

        x_flip = torch.flip(x, dims=[3])
        f6_r, f9_r, f11_r = self.forward_single_branch(x_flip)
        f6_r = self.avgpool(f6_r).flatten(1)
        f9_r = self.avgpool(f9_r).flatten(1)
        f11_r = self.avgpool(f11_r).flatten(1)
        feat_flip = torch.cat([f6_r, f9_r, f11_r], dim=1)
...
def train_one_experiment(config):
    ...
    model = BrainTumorViTTiny(
        num_classes=config['num_classes'],
        use_ca=config['use_ca'],
        use_symmetry=config['use_symmetry'],
        model_name=config.get('vit_model_name', 'vit_tiny_patch16_224'),
    ).to(device)
