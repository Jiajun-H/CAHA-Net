...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',          
    'log_dir': './logs_resnet152',    
    'save_dir': './checkpoints_resnet152',
    'num_classes': 4,
    'batch_size': 64,       
    'lr': 1e-3,             
    'epochs': 60,          
...
# ================= 4. ResNet152-based 模型实现 (从头训练) =================
class BrainTumorResNet152(nn.Module):
    def __init__(self, num_classes=4, use_dcn=True, use_ca=True, use_symmetry=True):
        super(BrainTumorResNet152, self).__init__()
        self.use_symmetry = use_symmetry
        print(f">>> 初始化 ResNet152 模型 | DCN: {use_dcn} | CA: {use_ca} | Sym: {use_symmetry}")

        base_model = models.resnet152(weights=None)

        if use_dcn:
            try:
                replace_conv_with_dcn(base_model.layer3)
                replace_conv_with_dcn(base_model.layer4)
            except Exception as e:
                print(f"DCN 替换失败: {e}")
