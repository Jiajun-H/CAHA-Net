...
try:
    ...
    raise RuntimeError("未安装 timm，无法创建 Inception-V4 backbone。请先安装: pip install timm") from e
...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_inceptionv4',
    'save_dir': './checkpoints_inceptionv4',
    'num_classes': 4,
    'batch_size': 64,
    'lr': 1e-3,
    'epochs': 60,
    ...
    # Inception-V4 相关
    'inception_model_name': 'inception_v4',
}
...
def fmix_data(data, targets, alpha=1.0, decay_power=3.0, shape=(299, 299), device='cuda'):
...
# ================= 3. 效率评估函数 =================
def measure_efficiency(model, device, input_size=(1, 3, 299, 299)):
    model.eval()
...
# ================= 4. Inception-V4-based 模型实现 =================
def _resolve_inception_model_name(model_name: str) -> str:
    """验证并解析 Inception 模型名称"""
    try:
...
class BrainTumorInceptionV4(nn.Module):
    """
    基于 Inception-V4 的脑肿瘤分类模型
    特点:
    - 使用 timm 的 features_only 模式提取多尺度特征
    - 支持 Coordinate Attention (CA)
    - 支持对称性分析 (Symmetry)
    - 多尺度特征融合
    """
    def __init__(self, num_classes=4, use_ca=True, use_symmetry=True, model_name='inception_v4'):
        super().__init__()
        self.use_symmetry = use_symmetry

        resolved_name = _resolve_inception_model_name(model_name)
        print(f">>> 初始化 Inception-V4 | model={resolved_name} | CA: {use_ca} | Sym: {use_symmetry}")

        # features_only 输出多个 stage 特征，取 3 个尺度
        # Inception-V4 典型输出索引: 0, 1, 2, 3, 4
        # 我们选择后3个尺度进行多尺度融合
        ...
        # 获取各 stage 的通道数
        try:
            self._feat_channels = list(self.backbone.feature_info.channels())  # [ch2, ch3, ch4]
        except Exception:
        ...
        print(f">>> Inception-V4 特征通道: Stage2={ch2}, Stage3={ch3}, Stage4={ch4}")
...
def train_one_experiment(config):
    ...
    model = BrainTumorInceptionV4(
        num_classes=config['num_classes'],
        use_ca=config['use_ca'],
        use_symmetry=config['use_symmetry'],
        model_name=config.get('inception_model_name', 'inception_v4'),
    ).to(device)
