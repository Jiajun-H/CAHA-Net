...
# 复用 model_utils 中的 CoordAtt 与 DCN 替换函数
...
# ================= 4. AlexNet-based 模型实现 (接口与原模型一致) =================
class BrainTumorAlexNet(nn.Module):
    def __init__(self, num_classes=4, use_dcn=True, use_ca=True, use_symmetry=True):
        super(BrainTumorAlexNet, self).__init__()
        self.use_symmetry = use_symmetry
        print(f">>> 初始化 AlexNet 模型 | DCN: {use_dcn} | CA: {use_ca} | Sym: {use_symmetry}")
        ...
        # 划分层次，选取三个尺度特征
        self.initial_layers = nn.Sequential(*list(features[:3]))  # conv0 + relu + pool
        self.block1 = nn.Sequential(*list(features[3:6]))        # conv1 + relu + pool
        self.block2 = nn.Sequential(*list(features[6:9]))        # conv2 + relu + conv
        self.block3 = nn.Sequential(*list(features[9:12]))       # conv3 + relu + conv + relu + pool

        # 动态推断各层输出通道数，保证 CoordAtt 使用正确的通道
        try:
            with torch.no_grad():
                dummy = torch.zeros(1, 3, 224, 224)
                x = self.initial_layers(dummy)
                x = self.block1(x)
                ch2 = x.size(1)
                x = self.block2(x)
                ch3 = x.size(1)
...
# ================= 5. 训练逻辑（与 train_album.py 保持一致，但使用 AlexNet 模型） =================
