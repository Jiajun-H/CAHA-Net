"""
Train UNet2D Classifier - 基于 LucaLumetti/UNetTransplant
参考: https://github.com/LucaLumetti/UNetTransplant

输出格式与 train_album.py 完全一致，作为对比实验模型

用法:
    python train_unet_classifier.py
"""
...
# 导入 UNet2D 分类器
from unet_classifier import UNet2DClassifier, ResidualUNet2DClassifier

# 尝试导入 thop 用于计算 GFLOPs
try:
...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_unet_classifier',
    'save_dir': './checkpoints_unet_classifier',
    'num_classes': 4,
    'batch_size': 64,
    'lr': 1e-3,
    'epochs': 60,
...
def train_one_experiment(config):
    ...
    # --- 生成实验名称 ---
    model_type = config.get('model_type', 'UNet2D')
    name_parts = [model_type]
    
    if config.get('use_clahe', False):
        name_parts.append("CLAHE")
    if config.get('use_fmix', False):
        name_parts.append("FMix")
    if config.get('use_multi_scale', True):
        name_parts.append("MultiScale")
    ...
    train_transform = transforms.Compose([
        ...
        AlbumentationsTransform(is_train=True, use_clahe=config.get('use_clahe', False)),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    val_transform = transforms.Compose([
        transforms.Resize((224, 224)),
        AlbumentationsTransform(is_train=False, use_clahe=config.get('use_clahe', False)),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    ...
    if model_type == 'UNet2D':
        ...
        model = ResidualUNet2DClassifier(
            in_channels=3,
            num_classes=config['num_classes'],
            f_maps=config.get('f_maps', 64),
            num_levels=config.get('num_levels', 4),
            use_multi_scale=config.get('use_multi_scale', True),
            dropout_prob=config.get('dropout_prob', 0.1),
        ).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        for i, (imgs, targets) in enumerate(train_loader):
            ...
            do_fmix = config.get('use_fmix', False) and (np.random.rand() > 0.5)
        ...
        print(f"\nVal ({exp_name}) -> ACC: {acc:.4f} | F1: {macro_f1:.4f} | AUC: {auc:.4f}")
        ...
        if macro_f1 > best_macro_f1:
            ...
            # 写入 TXT (与 train_album.py 格式一致)
            with open(os.path.join(current_save_dir, txt_filename), "w", encoding="utf-8") as f:
                f.write(f"Experiment:       {exp_name}\n")
                f.write(f"Model Type:       {model_type} (UNetTransplant-based)\n")
                f.write(f"Reference:        https://github.com/LucaLumetti/UNetTransplant\n")
...
def run_all_experiments():
    ...
    运行所有 UNet 对比实验
    可以选择不同的模型配置：
    - UNet2D: 标准 UNet 2D 分类器
    - ResUNet2D: 残差 UNet 2D 分类器
    """
    experiment_queue = [
        # 1. 标准 UNet2D (基线)
        {
        ...
        # 3. Residual UNet2D (基线)
        {
            'model_type': 'ResUNet2D',
            'f_maps': 64,
            'num_levels': 4,
            'use_clahe': False,
            'use_fmix': False,
