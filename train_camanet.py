...
BASE_CONFIG = {
    ...
    'val_dir': './data/val',
    'log_dir': './logs_camanet',
    'save_dir': './checkpoints_camanet',
    'num_classes': 4,
    'batch_size': 64,
    'lr': 1e-3,
    'epochs': 60,
...
# ================= 4. CAMANet Backbone (按你现有逻辑：三尺度 + 可选 CA/DCN/Sym) =================
class BrainTumorCAMANet(nn.Module):
    def __init__(self, num_classes=4, use_dcn=True, use_ca=True, use_symmetry=True):
        super().__init__()
        self.use_symmetry = use_symmetry

        # 这里的 CAMANet 定义：DenseNet121 backbone + 三尺度特征聚合(MA) + CoordAtt(CA) + 可选 Sym。
        # 训练管线/指标/保存方式保持与其他脚本一致。
        print(f">>> 初始化 CAMANet | DCN: {use_dcn} | CA: {use_ca} | Sym: {use_symmetry}")

        base_model = models.densenet121(weights=None)
        features = base_model.features

        if use_dcn:
            replace_conv_with_dcn(features.denseblock3)
            replace_conv_with_dcn(features.denseblock4)
