...
# ================= 4. DINOv3 (ViT) 模型实现 (从头训练) =================
class BrainTumorDINOv3(nn.Module):
    def __init__(self, num_classes=4, use_dcn=True, use_ca=True, use_symmetry=True):
        super().__init__()
        self.use_symmetry = use_symmetry

        if use_dcn:
            print("⚠️ DINOv3 为 Transformer 结构，DCN 选项将被忽略。")

        print(f">>> 初始化 DINOv3-ViT 模型 | CA: {use_ca} | Sym: {use_symmetry}")

        # timm 的模型名会随版本变化，这里做多候选自动匹配
        # 新版 timm 使用 vit_*_patch16_dinov3 格式
        model_candidates = [
        ...
        if backbone is None:
            # 给出清晰提示：需要升级 timm 或安装支持 DINOv3 的版本
            dinov3_list = []
            try:
                dinov3_list = timm.list_models('*dinov3*')
            except Exception:
                dinov3_list = []

            raise RuntimeError(
                "无法创建 DINOv3 backbone（当前 timm 版本可能不包含 DINOv3）。\n"
                "建议：\n"
                "1) 先升级 timm：pip install -U timm\n"
                "2) 查看可用模型名：python -c \"import timm; print(timm.list_models('*dinov3*'))\"\n"
                f"timm 检索到的 dinov3 模型名: {dinov3_list}\n"
                f"最后一次错误: {last_err}"
            )
        ...
        try:
            ...
            # 兜底（不同模型配置可能不同）
            ch2, ch3, ch4 = 384, 768, 1024

        print(f">>> DINOv3 backbone: {self.backbone_name} | channels: {ch2}, {ch3}, {ch4}")
    ...
    def forward_single_branch(self, x):
        ...
        feat2 = self.ca2(feats[0])
        feat3 = self.ca3(feats[1])
        feat4 = self.ca4(feats[2])
    ...
    for epoch in range(config['epochs']):
        ...
        if macro_f1 > best_macro_f1:
            ...
            with open(os.path.join(current_save_dir, txt_filename), "w", encoding="utf-8") as f:
                ...
                f.write(f"Backbone:         {getattr(model, 'backbone_name', 'dinov3')}\n")
